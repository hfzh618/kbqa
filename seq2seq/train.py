...
def get_batch(pairs, batch_size):
    ...
    source_batch = Variable(torch.LongTensor(source_batch)).transpose(0,
                                                                      1)  # (batch_size x max_len) tensors, transpose into (max_len x batch_size)
...
def run_epoch(source_batch, source_lengths, target_batch, target_lengths, encoder, decoder, encoder_optimizer=None,
              decoder_optimizer=None, TRAIN=True):
    if TRAIN:
...
if __name__ == "__main__":
    ...
    for iter in range(0, num_epoch):
        ...
        loss = run_epoch(source_batch, source_lengths, target_batch, target_lengths, encoder, decoder,
        ...
        if iter % print_every == 0:
            ...
            precision, preds = run_epoch(source_batch, source_lengths, target_batch, target_lengths, encoder, decoder,
